...
JobManager = (function() {
	...
	*/
	JobManager.prototype.load = function (filename, callback) {
		...
		configLoader.load(filename, function(err, config){
			...
			for (i = 0; i < config.jobs.length; i++){
				this.addJob(job.sanitizeConfig(config.jobs[i]));
			}
			this.emit('loaded');
		}.bind(this));
	};
	...
	*/
	JobManager.prototype.addJob = function (config) {
		var job;
		...
		this._jobsTasks[job.id] = {
			config: job.sanitizeConfig(config, false),
			tasks: {},
			doneCount: 0
		};
	...
	* @returns {Number}
	*/
	JobManager.prototype.jobCount = function() {
	...
	* {NetJob} job
	*/
	JobManager.prototype._onTaskStart = function(config, task, job) {
	...
	* {NetJob} job
	*/
	JobManager.prototype._onTaskProgress = function(config, task, message, job) {
		this._jobsTasks[job.id].tasks[task.id] = this.toDataItem({
				task: task,
				config: config,
				message: message
	...
	* {NetJob} job
	*/
	JobManager.prototype._onTaskResult = function(err, config, response, task, job) {
		this._jobsTasks[job.id].tasks[task.id] = this.toDataItem({
				task: task,
				err: err,
				config: config,
	...
	JobManager.prototype._onJobDone = function(job) {
		this._jobsTasks[job.id].doneCount = job.getDoneCount();
		this.emit('jobdone', job);
	};

	JobManager.prototype._onJobAbort = function(job) {
	...
	*/
	JobManager.prototype.jobsAreTerminated = function() {
		var jobId;
		if (this.jobCount() === 0) {
			return true;
		}
	...
	JobManager.prototype.toDataItem = function(config) {
		config = config || {};
		var data, i, keys;
	...
	*
	*/
	JobManager.prototype.abort = function() {
		var jobId;
		for (jobId in this._jobs) {
			if (this._jobs.hasOwnProperty(jobId)) {
	...
	* @private
	*/
	JobManager.prototype._eexception = function(exception) {
		var error;
		if (exception instanceof Error) {
			error = exception;
		} else {
			error = new Error(exception.message);
			Error.captureStackTrace(error, JobManager.prototype._eexception); // we do not trace this function
	...
	* @method
	*/
	JobManager.prototype._log = function() {
		if (!this.verbose) {
			return;
		}
		var args = arguments,
	...
	*
	*/
	JobManager.prototype.jobsTasks = function() {
