...
*/
TaskSetCollection = function(tasks, parentId) {
	events.EventEmitter.call(this);

	tasks = TaskSetCollection.sanitizeConfig(tasks);
...
util.inherits(TaskSetCollection, events.EventEmitter);


TaskSetCollection.prototype.run = function(){
	var
	i,
	taskSetConfig,
	taskSet,
	...
	for (i = 0; i < this.tasks.length; i++) {
		taskSetConfig = task_set.sanitizeConfig(this.tasks[i]);
		...
		taskSet = task_set.create(taskSetConfig);
...
TaskSetCollection.sanitizeConfig = function(config) {
	var
	err,
	cleanConfig;
...
* @param {NetTask} task
*/
TaskSetCollection.prototype._onTaskStart = function (config, task) {
...
* @param {Object} msg
*/
TaskSetCollection.prototype._onTaskProgress = function (config, task, msg) {
...
* @param {NetTask} task
*/
TaskSetCollection.prototype._onTaskResult = function (err, config, response, task) {
...
*/
TaskSetCollection.prototype._onTaskSetDone = function (netTaskSet) {
...
/**
* TaskSetCollection only if verbose is positive
* @public
* @method
*/
TaskSetCollection.prototype._log = function() {
	if (!this.verbose) {
		return;
	}
	var args = arguments,
...
* @private
*/
TaskSetCollection.prototype._eexception = function(exception) {
	var error;
	if (exception instanceof Error) {
		error = exception;
	} else {
		error = new Error(exception.message);
		Error.captureStackTrace(error, TaskSetCollection.prototype._eexception); // we do not trace this function
...
exports.create = function(config, parentId) {
	return new TaskSetCollection(config, parentId);
};
