...
Task.sanitizeConfig = function(config){
	...
	if(!cleanConfig.action) {
		...
		err.code = 'EUNDEFINEDACTION';
		return err;
	}
	if(typeof cleanConfig.action  !== 'function') {
		err = new Error('action is not a function');
...
Task.prototype.run = function(){
	...
	this.action(this.id, this.config, this._done.bind(this, startTime), this._log.bind(this, startTime));
};
...
Task.prototype._log = function(startTime, msg) {
	...
	this.state = 'log';
	this.emit('tasklog', this.config, this, msg);
};

Task.prototype._done = function (startTime, err, response) {
	...
	this.state = 'result';
	this.emit('taskresult', err, response, this);
	
	this._runSubTaskSetCollection('complete');
	if (err) {
		this._debug('%s failed: %s', this.action, err.message);
		this._runSubTaskSetCollection('failure');
	} else {
		this._debug("%s succeed", this.action);
