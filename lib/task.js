...
Task.prototype.run = function(){
	...
	function start() {
		...
		plugin.run(me.id, me.config,  function (err, config, response) {
			...
			me._runSubTaskSetCollection('complete');
			if (err) {
				me.log('%s failed: %s', this.action, err.message);
				me._runSubTaskSetCollection('failure');
			} else {
				me.log("%s succeed", this.action);
				me._runSubTaskSetCollection('success');
			}
			if (me._subTaskSetCollections.length === 0) {
				me.state = 'done';
				me.emit('done', me);
			}
		}, function(msg) {
...
* @param {Object} msg
*/
Task.prototype._onTaskSetCollectionProgress = function (config, task, msg) {
